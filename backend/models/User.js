...
function encryptEmail(email) {
  ...
  try {
    const encryptionKey = process.env.ENCRYPTION_KEY; // 환경 변수에서 키를 가져옴
    if (!encryptionKey || encryptionKey.length !== 64) {
      throw new Error(
        "Invalid encryption key. Ensure it is a 32-byte hex string (64 characters)."
      );
    }
    ...
    return iv.toString("hex") + ":" + encrypted; // IV와 암호화된 데이터를 반환
  } catch (error) {
